#!/bin/bash
# Wrapper script for kopia SSD snapshot — adds timestamps to log output.
# The plist's StandardOutPath/StandardErrorPath capture all output here.

KOPIA_BIN="/opt/homebrew/bin/kopia"
CONFIG="/Users/dang/Library/Application Support/kopia/repository.config"

echo "=== $(date '+%Y-%m-%d %H:%M:%S') Starting kopia SSD snapshot ==="
